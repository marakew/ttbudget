...
unsigned int const SecCrcTable[256] =
{
...
unsigned int SectionCrc(unsigned char *buf, unsigned int len)
{
	...
	for (i = 0; i < len; i++)
	{
		Crc = (Crc << 8) ^ SecCrcTable[ (Crc >> 24) ^ *buf++ ];
	}
...
unsigned int const SecCSumMask[] =
{
	0,
	0xFF,
	0xFFFF,
...
void SectionCSum(unsigned int *SecCSum, unsigned int *SecData, unsigned int SecLen)
{
	...
	if (lastLen)
	{
		CSum ^= (*SecData & SecCSumMask[lastLen]);
	}
...
int SectionCSumCheck(unsigned char *SecData, unsigned int SecLen)
{
	...
	CSum = *(unsigned int *)&SecData[SecLen - 4];
	...
	SectionCSum(&CSum, SecData, SecLen - 4);
...
unsigned short IP_CSum(unsigned char *IPH, unsigned short iplen)
{
	...
	if (iplen > 1)
	{
		register unsigned int nwords;
		nwords = (iplen - 2)/2 + 1;

		do {
			sum += *(unsigned short *)iphdr;
			iphdr += 2;
			iplen -= 2;
			--nwords;
		} while(nwords);
