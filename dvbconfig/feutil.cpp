...
char * decodeTone(fe_tone_t tone)
{
	switch(tone)
	{
	case SEC_TONE_OFF:	return "off";
	case SEC_TONE_ON:	return "on";
...
char * decodePolar(fe_polarity_t polar)
{
	switch(polar)
	{
	case SEC_POLARITY_H:	return "Hor";
	case SEC_POLARITY_V:	return "Ver";
	default:		return "error";
	}
}
...
{
	if (strcmp(inner, "1/2") == 0)	return FEC_1_2;
	else
	if (strcmp(inner, "2/3") == 0)	return FEC_2_3;
	else
	if (strcmp(inner, "3/4") == 0)	return FEC_3_4;
	else
	if (strcmp(inner, "5/6") == 0)	return FEC_5_6;
	else
	if (strcmp(inner, "6/7") == 0)	return FEC_6_7;
	else
	if (strcmp(inner, "7/8") == 0)	return FEC_7_8;
	else
	if (strcmp(inner, "auto") == 0)	return FEC_AUTO;
}
